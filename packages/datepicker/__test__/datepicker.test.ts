...
import { componentSpec, createDatepickerElement, defineDatepickerElements, getPartSpec, type ComponentPartName } from "../src";
...
describe("@ariaui-web/datepicker", () => {
  afterEach(() => {
  ...
  it("declares a native web component spec for every separated package part", () => {
    ...
    expect(componentSpec.packageName).toBe("@ariaui-web/datepicker");
    expect(componentSpec.slug).toBe("datepicker");
  ...
  it("exposes helpers that resolve and create every spec part", () => {
    for (const part of componentSpec.parts) {
      ...
      const element = createDatepickerElement(part.name);
      expect(element.tagName.toLowerCase()).toBe(part.tagName);
    }

    expect(() => getPartSpec("__missing__" as ComponentPartName)).toThrow("Unknown @ariaui-web/datepicker part");
  });
  ...
  it("connects every custom element to its spec part metadata", () => {
    ...
    for (const part of componentSpec.parts) {
      ...
      expect(element.getAttribute("data-ariaui-web")).toBe("datepicker");
      expect(element.getAttribute("data-package")).toBe("datepicker");
