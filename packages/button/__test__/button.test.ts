...
import { componentSpec, createButtonElement, defineButtonElements, getPartSpec, type ComponentPartName } from "../src";
...
describe("@ariaui-web/button", () => {
  ...
  it("declares a native web component spec for every separated package part", () => {
    ...
    expect(componentSpec.packageName).toBe("@ariaui-web/button");
    expect(componentSpec.slug).toBe("button");
  ...
  it("exposes helpers that resolve and create every spec part", () => {
    for (const part of componentSpec.parts) {
      ...
      const element = createButtonElement(part.name);
      expect(element.tagName.toLowerCase()).toBe(part.tagName);
    }

    expect(() => getPartSpec("__missing__" as ComponentPartName)).toThrow("Unknown @ariaui-web/button part");
  });
  ...
  it("creates elements that reflect the common Aria UI Web contract", () => {
    ...
    expect(element.getAttribute("data-ariaui-web")).toBe("button");
  ...
  it("connects every custom element to its spec part metadata", () => {
    ...
    for (const part of componentSpec.parts) {
      ...
      expect(element.getAttribute("data-ariaui-web")).toBe("button");
      expect(element.getAttribute("data-package")).toBe("button");
