...
import { componentSpec, createKbdElement, defineKbdElements, getPartSpec, type ComponentPartName } from "../src";
...
describe("@ariaui-web/kbd", () => {
  ...
  it("declares a native web component spec for every separated package part", () => {
    ...
    expect(componentSpec.packageName).toBe("@ariaui-web/kbd");
    expect(componentSpec.slug).toBe("kbd");
  ...
  it("exposes helpers that resolve and create every spec part", () => {
    for (const part of componentSpec.parts) {
      ...
      const element = createKbdElement(part.name);
      expect(element.tagName.toLowerCase()).toBe(part.tagName);
    }

    expect(() => getPartSpec("__missing__" as ComponentPartName)).toThrow("Unknown @ariaui-web/kbd part");
  });
  ...
  it("creates elements that reflect the common Aria UI Web contract", () => {
    defineKbdElements();
    const element = createKbdElement();
  ...
  it("connects every custom element to its spec part metadata", () => {
    ...
    for (const part of componentSpec.parts) {
      ...
      expect(element.getAttribute("data-ariaui-web")).toBe("kbd");
      expect(element.getAttribute("data-package")).toBe("kbd");
