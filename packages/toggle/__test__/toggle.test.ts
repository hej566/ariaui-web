...
import { componentSpec, createToggleElement, defineToggleElements, getPartSpec, type ComponentPartName } from "../src";
...
describe("@ariaui-web/toggle", () => {
  ...
  it("declares a native web component spec for every separated package part", () => {
    ...
    expect(componentSpec.packageName).toBe("@ariaui-web/toggle");
    expect(componentSpec.slug).toBe("toggle");
  ...
  it("exposes helpers that resolve and create every spec part", () => {
    for (const part of componentSpec.parts) {
      ...
      const element = createToggleElement(part.name);
      expect(element.tagName.toLowerCase()).toBe(part.tagName);
    }

    expect(() => getPartSpec("__missing__" as ComponentPartName)).toThrow("Unknown @ariaui-web/toggle part");
  });
  ...
  it("connects every custom element to its spec part metadata", () => {
    ...
    for (const part of componentSpec.parts) {
      ...
      expect(element.getAttribute("data-ariaui-web")).toBe("toggle");
      expect(element.getAttribute("data-package")).toBe("toggle");
