...
import { componentSpec, createNavigationMenuElement, defineNavigationMenuElements, getPartSpec, type ComponentPartName } from "../src";
...
describe("@ariaui-web/navigation-menu", () => {
  afterEach(() => {
  ...
  it("declares a native web component spec for every separated package part", () => {
    ...
    expect(componentSpec.packageName).toBe("@ariaui-web/navigation-menu");
    expect(componentSpec.slug).toBe("navigation-menu");
  ...
  it("exposes helpers that resolve and create every spec part", () => {
    for (const part of componentSpec.parts) {
      ...
      const element = createNavigationMenuElement(part.name);
      expect(element.tagName.toLowerCase()).toBe(part.tagName);
    }

    expect(() => getPartSpec("__missing__" as ComponentPartName)).toThrow("Unknown @ariaui-web/navigation-menu part");
  });
  ...
  it("connects every custom element to its spec part metadata", () => {
    ...
    for (const part of componentSpec.parts) {
      ...
      expect(element.getAttribute("data-ariaui-web")).toBe("navigation-menu");
      expect(element.getAttribute("data-package")).toBe("navigation-menu");
