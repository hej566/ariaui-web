...
import { componentSpec, createFocusScopeElement, defineFocusScopeElements, getPartSpec, type ComponentPartName } from "../src";
...
describe("@ariaui-web/focus-scope", () => {
  afterEach(() => {
  ...
  it("declares a native web component spec for every separated package part", () => {
    ...
    expect(componentSpec.packageName).toBe("@ariaui-web/focus-scope");
    expect(componentSpec.slug).toBe("focus-scope");
    expect("sourcePackage" in componentSpec).toBe(false);
    expect(componentSpec.parts.length).toBeGreaterThan(0);
    expect(componentSpec.parts[0]?.name).toBe("FocusScope");
  ...
  it("exposes helpers that resolve and create every spec part", () => {
    for (const part of componentSpec.parts) {
      ...
      const element = createFocusScopeElement(part.name);
      expect(element.tagName.toLowerCase()).toBe(part.tagName);
    }

    expect(() => getPartSpec("__missing__" as ComponentPartName)).toThrow("Unknown @ariaui-web/focus-scope part");
  });
  ...
  it("creates elements that reflect the common Aria UI Web contract", () => {
    defineFocusScopeElements();
    const element = createFocusScopeElement();
    element.setAttribute("orientation", "horizontal");
    document.body.append(element);

    expect(element.tagName.toLowerCase()).toBe(componentSpec.parts[0]?.tagName);
    expect(element.getAttribute("data-ariaui-web")).toBe("focus-scope");
    expect(element.getAttribute("data-part")).toBe("FocusScope");
  ...
  it("connects every custom element to its spec part metadata", () => {
    ...
    for (const part of componentSpec.parts) {
      ...
      expect(element.getAttribute("data-ariaui-web")).toBe("focus-scope");
      expect(element.getAttribute("data-package")).toBe("focus-scope");
