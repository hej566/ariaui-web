...
import { componentSpec, createListboxElement, defineListboxElements, getPartSpec, type ComponentPartName } from "../src";
...
describe("@ariaui-web/listbox", () => {
  ...
  it("declares a native web component spec for every separated package part", () => {
    ...
    expect(componentSpec.packageName).toBe("@ariaui-web/listbox");
    expect(componentSpec.slug).toBe("listbox");
  ...
  it("exposes helpers that resolve and create every spec part", () => {
    for (const part of componentSpec.parts) {
      ...
      const element = createListboxElement(part.name);
      expect(element.tagName.toLowerCase()).toBe(part.tagName);
    }

    expect(() => getPartSpec("__missing__" as ComponentPartName)).toThrow("Unknown @ariaui-web/listbox part");
  });
