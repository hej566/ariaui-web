...
import { componentSpec, createArrowElement, defineArrowElements, getPartSpec, type ComponentPartName } from "../src";
...
describe("@ariaui-web/arrow", () => {
  ...
  it("declares a native web component spec for every separated package part", () => {
    ...
    expect(componentSpec.packageName).toBe("@ariaui-web/arrow");
    expect(componentSpec.slug).toBe("arrow");
  ...
  it("exposes helpers that resolve and create every spec part", () => {
    for (const part of componentSpec.parts) {
      ...
      const element = createArrowElement(part.name);
      expect(element.tagName.toLowerCase()).toBe(part.tagName);
    }

    expect(() => getPartSpec("__missing__" as ComponentPartName)).toThrow("Unknown @ariaui-web/arrow part");
  });
  ...
  it("creates elements that reflect the common Aria UI Web contract", () => {
    ...
    expect(element.getAttribute("data-ariaui-web")).toBe("arrow");
  ...
  it("connects every custom element to its spec part metadata", () => {
    ...
    for (const part of componentSpec.parts) {
      ...
      expect(element.getAttribute("data-ariaui-web")).toBe("arrow");
      expect(element.getAttribute("data-package")).toBe("arrow");
