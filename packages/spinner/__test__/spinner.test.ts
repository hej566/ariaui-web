...
import { componentSpec, createSpinnerElement, defineSpinnerElements, getPartSpec, type ComponentPartName } from "../src";
...
describe("@ariaui-web/spinner", () => {
  ...
  it("exposes helpers that resolve and create every spec part", () => {
    for (const part of componentSpec.parts) {
      ...
      const element = createSpinnerElement(part.name);
      expect(element.tagName.toLowerCase()).toBe(part.tagName);
    }

    expect(() => getPartSpec("__missing__" as ComponentPartName)).toThrow("Unknown @ariaui-web/spinner part");
  });
  ...
  it("creates elements that reflect the common Aria UI Web contract", () => {
    ...
    expect(element.getAttribute("data-ariaui-web")).toBe("spinner");
  ...
  it("connects every custom element to its spec part metadata", () => {
    ...
    for (const part of componentSpec.parts) {
      ...
      expect(element.getAttribute("data-ariaui-web")).toBe("spinner");
      expect(element.getAttribute("data-package")).toBe("spinner");
