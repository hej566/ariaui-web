...
describe("@ariaui-web/aspect-ratio", () => {
  afterEach(() => {
  ...
  it("declares a native web component spec for every separated package part", () => {
    ...
    expect(componentSpec.packageName).toBe("@ariaui-web/aspect-ratio");
    expect(componentSpec.slug).toBe("aspect-ratio");
  ...
  it("exposes helpers that resolve and create every spec part", () => {
    for (const part of componentSpec.parts) {
      ...
      const element = createAspectRatioElement(part.name);
      expect(element.tagName.toLowerCase()).toBe(part.tagName);
    }

    expect(() => getPartSpec("__missing__" as ComponentPartName)).toThrow("Unknown @ariaui-web/aspect-ratio part");
  });
  ...
  it("connects every custom element to its spec part metadata", () => {
    ...
    for (const part of componentSpec.parts) {
      ...
      expect(element.getAttribute("data-ariaui-web")).toBe("aspect-ratio");
      expect(element.getAttribute("data-package")).toBe("aspect-ratio");
