...
import { componentSpec, createTreegridElement, defineTreegridElements, getPartSpec, type ComponentPartName } from "../src";
...
describe("@ariaui-web/treegrid", () => {
  ...
  it("declares a native web component spec for every separated package part", () => {
    ...
    expect(componentSpec.packageName).toBe("@ariaui-web/treegrid");
    expect(componentSpec.slug).toBe("treegrid");
  ...
  it("exposes helpers that resolve and create every spec part", () => {
    for (const part of componentSpec.parts) {
      ...
      const element = createTreegridElement(part.name);
      expect(element.tagName.toLowerCase()).toBe(part.tagName);
    }

    expect(() => getPartSpec("__missing__" as ComponentPartName)).toThrow("Unknown @ariaui-web/treegrid part");
  });
  ...
  it("connects every custom element to its spec part metadata", () => {
    ...
    for (const part of componentSpec.parts) {
      ...
      expect(element.getAttribute("data-ariaui-web")).toBe("treegrid");
      expect(element.getAttribute("data-package")).toBe("treegrid");
