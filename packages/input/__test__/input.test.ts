...
import { componentSpec, createInputElement, defineInputElements, getPartSpec, type ComponentPartName } from "../src";
...
describe("@ariaui-web/input", () => {
  ...
  it("declares a native web component spec for every separated package part", () => {
    ...
    expect(componentSpec.packageName).toBe("@ariaui-web/input");
    expect(componentSpec.slug).toBe("input");
  ...
  it("exposes helpers that resolve and create every spec part", () => {
    for (const part of componentSpec.parts) {
      ...
      const element = createInputElement(part.name);
      expect(element.tagName.toLowerCase()).toBe(part.tagName);
    }

    expect(() => getPartSpec("__missing__" as ComponentPartName)).toThrow("Unknown @ariaui-web/input part");
  });
  ...
  it("creates elements that reflect the common Aria UI Web contract", () => {
    ...
    expect(element.getAttribute("data-ariaui-web")).toBe("input");
  ...
  it("connects every custom element to its spec part metadata", () => {
    ...
    for (const part of componentSpec.parts) {
      ...
      expect(element.getAttribute("data-ariaui-web")).toBe("input");
      expect(element.getAttribute("data-package")).toBe("input");
