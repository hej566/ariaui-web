...
import { componentSpec, createSkeletonElement, defineSkeletonElements, getPartSpec, type ComponentPartName } from "../src";
...
describe("@ariaui-web/skeleton", () => {
  ...
  it("exposes helpers that resolve and create every spec part", () => {
    for (const part of componentSpec.parts) {
      ...
      const element = createSkeletonElement(part.name);
      expect(element.tagName.toLowerCase()).toBe(part.tagName);
    }

    expect(() => getPartSpec("__missing__" as ComponentPartName)).toThrow("Unknown @ariaui-web/skeleton part");
  });
  ...
  it("creates elements that reflect the common Aria UI Web contract", () => {
    ...
    expect(element.getAttribute("data-ariaui-web")).toBe("skeleton");
  ...
  it("connects every custom element to its spec part metadata", () => {
    ...
    for (const part of componentSpec.parts) {
      ...
      expect(element.getAttribute("data-ariaui-web")).toBe("skeleton");
      expect(element.getAttribute("data-package")).toBe("skeleton");
